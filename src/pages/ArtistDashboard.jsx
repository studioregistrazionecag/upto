...
const ArtistDashboard = () => {
    ...
    const fetchEventsAndBookings = async () => {
        ...
        const { data: eventData, error: eventError } = await supabase
            .from('events')
            .select(`id, event_date, time_slots (id, start_time, end_time, status, is_private, artist_id, artist:profiles!time_slots_artist_id_fkey(full_name), producer:profiles!time_slots_producer_id_fkey(full_name))`)
    ...
    const executeCancellation = async () => {
        ...
        // Quando si annulla, resettiamo anche is_private a false
                        ...
                        <div key={event.id} className={styles.eventCard} style={{ animationDelay: `${index * 100}ms` }}>
                            ...
                            <div className={styles.slotsContainer}>
                                {event.time_slots
                                    ...
                                    .map(slot => {
                                        const isMyBooking = slot.artist_id === userId;
                                        // AGGIORNATO: Classe condizionale per slot privato
                                        return (
